...
function fileQueueError(file, errorCode, message) {
	try {
		...
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			errorName = "你选择的文件个数超除了最大数"+swfu.settings.file_upload_limit;
		}
...
function fileQueued(file){
	$("#thumbnails").show();
	addReadyFileInfo(file.id,file.name,"Successfully loaded to upload queue","Ready to upload");
	//swfu.startUpload();
}
...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		if (percent === 100) {
			...
			addFileInfo(file.id,"Uploading..."+percent+"%");
...
function uploadSuccess(file, serverData) {
	try {
		var progress = new FileProgress(file,  this.customSettings.upload_target);
		addFileInfo(file.id,"Upload complete");
		addFileId(file.id,serverData);
		$("#"+file.id+" a").css("visibility","hidden");
	} catch (ex) {
		this.debug(ex);
	}
}
...
function addFileInfo(fileId,message){
	var row = document.getElementById(fileId);
	row.cells[2].innerHTML = "<font class='green' color='green'>"+message+"</font>";
}
function addReadyFileInfo(fileid,fileName,message,status){
	...
	if(status!=null&&status!=""){
		col3.innerHTML="<font class='red' color='red'>"+status+"</font>";
	}else{
		col3.innerHTML="";
	}
	col4.innerHTML = "<a style='visibility:visible;' href='javascript:deleteFile(\""+fileid+"\")'>Delete</a>";
...
function stop() {
	...
	$.ajax({
		type : 'post',
		url : "VideoUploadAction.go?upload=del",
		...
		error:function(data){
			addFileInfo(fileId,"<font color='red'>Delete folder error</a>");
		}
	});
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			progress.setStatus("<font color='red'>All files have been uploaded!</b></font>");
...
function uploadError(file, errorCode, message) {
	var imageName =  "<font color='red'>File upload error!</font>";
	var progress;
	try {
		switch (errorCode) {
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			try {
				...
				progress.setStatus("<font color='red'>Upload cancelled!</font>");
				progress.toggleCancel(false);
			}
			catch (ex1) {
				this.debug(ex1);
		...
		case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
			try {
				progress = new FileProgress(file,  this.customSettings.upload_target);
				progress.setCancelled();
				progress.setStatus("<font color='red'>Upload Stop!</font>");
		...
		case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
			imageName = "<font color='red'>File size limit exceeded!</font>";
			break;
		default:
			alert(message);
...
function FileProgress(file, targetID) {
	...
	if (!this.fileProgressWrapper) {
		...
		progressText.appendChild(document.createTextNode("Upload file: "+file.name));
		...
		this.fileProgressElement.childNodes[1].firstChild.nodeValue = "Upload file: "+file.name;
	}
