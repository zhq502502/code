...
<div id="wpbody-content">
	<div class="wrap">
		...
		<div id="tabs" class="tab_m">
			...
			<li><a href="#tabs-2"><%=lu.getLanguage(language,"confedit.confcommon","默认与会者") %></a></li>
			...
			<form>	
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabnew" id='tabnew'>
					<thead>
						...
						<tr>
							...
							<td width="47%" class="tdTitle"><%=lu.getLanguage(language,"confedit.confcommon","默认与会者") %></td>
					...
					<tbody>
					    ...
					    <c:if test="${fn:length(commonList)==0}"><tr><td colspan="2"><%=lu.getLanguage(language,"list.norecord","没有记录") %></td></tr></c:if>							
					    <c:forEach var="cList" items="${commonList}" varStatus="id">
					     <tr bgcolor="#FFFFFF" id="${cList.userId}">
					         <td width="6%"><input name="usernames" type="checkbox" class="checkchil" value="${cList.userName}"/></td>
							<td width="47%">${cList.userName}</td>
							<td width="47%">						
                               <span><img src='images_gb/del.gif' width='16' height='16' class='img_tab' /><a class='removecommon' href='javascript:void(0);'><%=lu.getLanguage(language,"button.delete","删除") %></a></span>                              
                           </td>
						</tr>
						</c:forEach>
						</table></td></tr>
				...
				<div id="sabrosus" class="sabrosus">
		                    ...
		                    <c:if test="${nowPage>1}">
		                       <a href="MeetingModCommonAction.go?page=1&confid=${confid}"><<</a>
		                       <a href="MeetingModCommonAction.go?page=${nowPage-1}&confid=${confid}"><</a>
		                    </c:if>
		                    <% 
			                     //总页数
		                         ...
		                         if(nowNumber <= 6 || pageNumber <= 10){
		                         	for(int i=1;i<=10 && i<= pageNumber;i++){
		                       			if(i==nowNumber){
		                           			...
		                           			out.println("<a href=MeetingModCommonAction.go?page=" + i + "&confid="+confid+">" + i + "</a>");
		                           		}
		                         	}
		                         }
		                    ...
		                    <c:if test="${nowPage!=pageNumber}">
		                        <a href="MeetingModCommonAction.go?page=${nowPage+1}&confid=${confid}">></a>
		                        <a href="MeetingModCommonAction.go?page=${pageNumber}&confid=${confid}">>></a>
		                        ${nowPage}/${pageNumber} 
		                    </c:if>
		...
		<tr>
			<td width="180" >
				<div id="linkman_left" class="bd_upload" style="border: 1px solid #C6D5E1;width: 100%;height: 280px;overflow: auto;">
					<ul id="leftUl">
					      <c:forEach var="cList1" items="${ucommonList}" varStatus="id">                                
                               <li onclick="leftClick(this)"><span id="${cList1.userName}" ailas="${cList1.alias}" class="m_text">${cList1.userName}[${cList1.alias}]</span><img class="m_img" align="absmiddle" src="images_gb/bot_add.gif"></li>                          
                          </c:forEach>
					</ul>
				</div>
			</td>
			...
			$('.removecommon').click(function(){
				...
				$.post(siteurl+"/CheckRoleServlet", { _: new Date().getTime() } ,function(rs) {
						if(rs==null||rs==""){
							...
							$.post(siteurl+"/ConfUserServlet", { action:'commonUser', account: account, cid: cid, operation: "confuserdelbyname",_: new Date().getTime() } ,function(rs) {
								if(rs==0){
									alert(getMsg(CANCEL_CONFCOMMON_SUCESS));
									var page = "${nowPage}";
									location.href=siteurl+'/ConfPage.go?inc=ConfCommon&confid='+cid+'&page='+page;
								} else {
									alert(getMsg(CANCEL_CONFCOMMON_FAILURE));
									return false;
								}
							});	
						} 
			...
			if(confirm(getMsg(CONFIRM_DELETE))){
				$.post(siteurl+"/CheckRoleServlet", { _: new Date().getTime() } ,function(rs) {
					if(rs==null||rs==""){
						...
						$.post(siteurl+"/ConfUserServlet", { action:'commonUser', account: usernames, cid: cid, operation: "confuserdelbyname",_: new Date().getTime() } ,function(rs) {
							if(rs==0){
								alert(getMsg(CANCEL_CONFCOMMON_SUCESS));
								var page = "${nowPage}";
								location.href=siteurl+'/ConfPage.go?inc=ConfCommon&confid='+cid+'&page='+page;
	...
	<script type="text/javascript">
		...
		function selectUsersOpen(){
			$('#dialog_box').dialog({
				...
				width:455,
				height:430,
				title:"<span style='color:#FFFFFF;'>"+getMsg(ERR_NO_CONFCOMMON_SELECTED)+"</span>"
		...
		function okselect(){
			...
			rightLi.each(function(i,n){
				var name = $(this).attr("id")+";"+$(this).attr("ailas");
				names+=","+name;
			})
			names = names.substring(1,names.length)            
			var cid = $('#cid').val();
			$.post(siteurl+"/ConfUserServlet", { action:'commonUser', account: names, cid: cid, operation: "confuseraddbyname",_: new Date().getTime() } ,function(rs) {
				var page = "${nowPage}";
				location.href=siteurl+'/ConfPage.go?inc=ConfCommon&confid='+cid+'&page='+page;
            });	            
			closeDlog();
