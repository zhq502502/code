		...
		<div class="wrap">
			<div class="space_03">
				...
				<div id="tabs" class="tab_m ui-tabs ui-widget ui-widget-content ui-corner-all">
					...
					<li><a href="#tabs-11" onclick="yonghugl()"><%=lu.getLanguage(language,"confsetmanager.setusermanager","设置用户管理员") %></a></li>
					<li><a href="#tabs-12" ><%=lu.getLanguage(language,"confsetmanager.setconfmanager","设置会议管理员") %></a></li>
					</ul>
					...
					<div id="tabs-12">
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							...
							class="tabcontent" id='tabcontent'>
								<c:if test="${fn:length(cmList)==0}"><tr><td colspan="2"><%=lu.getLanguage(language,"list.norecord","没有记录") %></td></tr></c:if>							
								...
								<tr id="${user.userId}">
									...
									<span>
										...
										<a class='removeadmin' href='javascript:void(0);'><%=lu.getLanguage(language,"topusersetmanager.cancelmanager","取消会议管理员") %></a>
									</span>
									</td>
								</tr>
		                      	</c:forEach>
						...
						<div id="sabrosus" class="sabrosus">
		                    ...
		                    <c:if test="${nowPage>1}">
		                       <a href="ConfSetManagerAction.go?page=1"><<</a>
		                       <a href="ConfSetManagerAction.go?page=${nowPage-1}"><</a>
		                    </c:if>
		                    <% 
			                     //总页数
		                         ...
		                         if(nowNumber <= 6 || pageNumber <= 10){
		                         	for(int i=1;i<=10 && i<= pageNumber;i++){
		                       			if(i==nowNumber){
		                           			...
		                           			out.println("<a href=ConfSetManagerAction.go?page=" + i + ">" + i + "</a>");
		                           		}
		                         	}
		                         }
		                    ...
		                    <c:if test="${nowPage!=pageNumber}">
		                        <a href="ConfSetManagerAction.go?page=${nowPage+1}">></a>
		                        <a href="ConfSetManagerAction.go?page=${pageNumber}">>></a>
		                        ${nowPage}/${pageNumber} 
		                    </c:if>
...
<div class='message'>
	...
	$(document).ready(function() {
		...
		$('.removeadmin').click(function(){
			if(confirm(getMsg(CONFIRM_CANCEL))){
				...
				$.post(siteurl+"/conf/confmanager", { action:'delconfmanager', account: account, role: "2",_: new Date().getTime() } ,function(rs) {
					if(rs==0){
						alert(getMsg(CANCEL_ADMIN_SUCCESS));
						var page = "${nowPage}";
						location.href=siteurl+'/ConfSetManagerAction.go?page='+page;
					} else {
		...
		if(confirm(getMsg(CONFIRM_CANCEL))){
			$.post(siteurl+"/CheckRoleServlet", { _: new Date().getTime() } ,function(rs) {
				if(rs==null||rs==""){
					...
					$.post(siteurl+"/conf/confmanager", { action:'delconfmanager', account: usernames, role: "2",_: new Date().getTime() } ,function(rs) {
						if(rs==0){
							alert(getMsg(CANCEL_ADMIN_SUCCESS));
							var page = "${nowPage}";
							location.href=siteurl+'/ConfSetManagerAction.go?page='+page;
	...
	function yonghugl(){
		var siteurl = $('#siteurl').val();
		location.href=siteurl+'/ConfPage.go?inc=UserSetManager';
	}
	</script>
	<script type="text/javascript">
		var siteurl = $('#siteurl').val();
		...
		function okselect(){
			...
			var cid = $('#cid').val();
			$.post(siteurl+"/conf/confmanager", { action:'addconfmanager', account: names, role: "2",_: new Date().getTime() } ,function(rs) {
