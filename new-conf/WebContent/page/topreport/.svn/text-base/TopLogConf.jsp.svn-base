...
$(document).ready(function() {
   ...
   function selectConfLog(){
      ...
      document.forms[0].action = "TopMeetingCountConfAction.go?begintime="+btime+"&endtime="+etime;
			...
			<div class="space_03">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabnew" id="tabnew">
					...
					<tbody>
					   ...
					   <c:forEach var="mccf" items="${ccList}" varStatus="id">
                         <tr bgcolor="#FFFFFF" id="${mccf.confid}">
							...
							%>
							<td width="10%"><a href="TopMeetingCountConfDetailAction.go?begintime=${begintime}&endtime=${endtime}&confid=${mccf.confid}"><img
				...
				<br />
				 <div class="sabrosus">
                    ...
                    <c:if test="${nowPage>1}">
                                <a href="TopMeetingCountConfAction.go?begintime=${begintime}&endtime=${endtime}&page=1"><<</a>
                                <a href="TopMeetingCountConfAction.go?begintime=${begintime}&endtime=${endtime}&page=${nowPage-1}"><</a>
                    </c:if>
                                ...
                                if(nowNumber <= 6 || pageNumber <= 10){
                                	for(int i=1;i<=10 && i<= pageNumber;i++){
                                			if(i==nowNumber){
                                                ...
                                                <a href="TopMeetingCountConfAction.go?begintime=${begintime}&endtime=${endtime}&page=<%=i%>"><%=i %></a>
                                     		<% }
                                		}
...
%>
                    <c:if test="${nowPage!=pageNumber}">
                                <a href="TopMeetingCountConfAction.go?begintime=${begintime}&endtime=${endtime}&page=${nowPage+1}">></a>
                                <a href="TopMeetingCountConfAction.go?begintime=${begintime}&endtime=${endtime}&page=${pageNumber}">>></a>
                                ${nowPage}/${pageNumber} 
                    </c:if>
