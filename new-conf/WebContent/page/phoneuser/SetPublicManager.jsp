		...
		<div class="wrap">
			<div class="space_03">
				...
				<div id="tabs" class="tab_m ui-tabs ui-widget ui-widget-content ui-corner-all">
					...
					<li><a href="#tabs-11"><%=lu.getLanguage(language,"phone.publicadmin","公共电话会议管理员") %></a></li>
					<li><a href="#tabs-12" onclick="personal()"><%=lu.getLanguage(language,"phone.personalconf","个人电话会议人员") %></a></li>
					</ul>
					...
					<div id="tabs-11">
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							...
							class="tabcontent" id='tabcontent'>
								<c:if test="${fn:length(publicadminList)==0}"><tr><td colspan="2"><%=lu.getLanguage(language,"list.norecord","没有记录") %>!</td></tr></c:if>							
								<c:forEach var="user" items="${publicadminList}" varStatus="id">
								<tr id="${user.userId}">
									...
									<span>
										...
										<a class='removeadmin' href='javascript:void(0);'><%=lu.getLanguage(language,"phone.cancelpublicadmin","取消公共管理员") %></a>
									</span>
									</td>
								</tr>
		                      	</c:forEach>
						...
						<div id="sabrosus" class="sabrosus">
		                    ...
		                    <c:if test="${nowPage>1}">
		                       <a href="SetPublicManagerAction.go?page=1"><<</a>
		                       <a href="SetPublicManagerAction.go?page=${nowPage-1}"><</a>
		                    </c:if>
		                    <% 
			                     //总页数
		                         ...
		                         if(nowNumber <= 6 || pageNumber <= 10){
		                         	for(int i=1;i<=10 && i<= pageNumber;i++){
		                       			if(i==nowNumber){
		                           			...
		                           			out.println("<a href=SetPublicManagerAction.go?page=" + i + ">" + i + "</a>");
		                           		}
		                         	}
		                         }
		                    ...
		                    <c:if test="${nowPage!=pageNumber}">
		                        <a href="SetPublicManagerAction.go?page=${nowPage+1}">></a>
		                        <a href="SetPublicManagerAction.go?page=${pageNumber}">>></a>
		                        ${nowPage}/${pageNumber} 
		                    </c:if>
...
<div class='message'>
		...
		<tr>
			<td width="180" >
				<div id="linkman_left" class="bd_upload" style="border: 1px solid #C6D5E1;width: 100%;height: 280px;overflow: auto;">
					<ul id="leftUl">
					      <c:forEach var="user1" items="${upublicadminList}" varStatus="id">                                    
	...
	<script type="text/javascript">
			...
			$('.removeadmin').click(function(){
				...
				$.post(siteurl+"/PhoneAdminServlet", { account: account, operation: "delPhoneAdmin",_: new Date().getTime() } ,function(rs) {
					if(rs==0){
						...
						var page = "${nowPage}";
						location.href=siteurl+'/SetPublicManagerAction.go?page='+page;
					} else {
		...
		function personal(){
			var siteurl = $('#siteurl').val();
			location.href=siteurl+'/ConfPage.go?inc=SetPersonalPhone';
		}
	</script>
	<script type="text/javascript">
		var siteurl = $('#siteurl').val();
		...
		function okselect(){
			...
			var cid = $('#cid').val();
			$.post(siteurl+"/PhoneAdminServlet", { account: names, type: "1", operation: "addPhoneAdmin",_: new Date().getTime() } ,function(rs) {				
				location.href=siteurl+'/SetPublicManagerAction.go';
            });	            
			closeDlog();
		}
		/*关闭弹出框*/
