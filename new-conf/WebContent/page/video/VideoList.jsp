...
<script type="text/javascript"> 
	...
	function selectConf(){
	    ...
	    document.forms[0].action = "VideoListAction.go?Keyword="+Keyword;
	    	...
	    	if(confirm(getMsg(CONFIRM_DELETE))){ 
	            $.post(siteurl+"/video/update", {action:'delete',id: id, _: new Date().getTime() } ,function(rs) {
	            	if(rs==0){
	             	...
	             	location.href=siteurl+'/ConfPage.go?inc=VideoList';
	            	}
	            	else {
				...
				<br />
				  <div class="sabrosus">
                    ...
                    <c:if test="${nowPage>1}">
                        <c:choose>
                            <c:when test="${Keyword!=''}">
                                <a href="VideoListAction.go?page=1&Keyword=${Keyword}"><<</a>
                                <a href="VideoListAction.go?page=${nowPage-1}&Keyword=${Keyword}"><</a>
                            </c:when>
                            <c:otherwise>
                                <a href="VideoListAction.go?page=1"><<</a>
                                <a href="VideoListAction.go?page=${nowPage-1}"><</a>
                            </c:otherwise>
                        </c:choose>
                    </c:if>
                                	...
                                	for(int i=1;i<=10 && i<= pageNumber;i++){
                                		if(Keyword != null && Keyword != "") {
                                			if(i==nowNumber){
                                    			...
                                    			out.println("<a href=VideoListAction.go?Keyword=" + Keyword + "&page=" + i + ">" + i + "</a>");
                                    		}
                                		}
                                		else {
                                			if(i==nowNumber){
                                    			...
                                    			out.println("<a href=VideoListAction.go?page=" + i + ">" + i + "</a>");
                                    		}
                                		}
                                	}
                                }
...
%>
                    <c:if test="${nowPage!=pageNumber}">
                        <c:choose>
                            <c:when test="${Keyword!=''}">
                                <a href="VideoListAction.go?page=${nowPage+1}&Keyword=${Keyword}">></a>
                                <a href="VideoListAction.go?page=${pageNumber}&Keyword=${Keyword}">>></a>
                                ${nowPage}/${pageNumber}
                            </c:when>
                            <c:otherwise>
                                <a href="VideoListAction.go?page=${nowPage+1}">></a>
                                <a href="VideoListAction.go?page=${pageNumber}">>></a>
                                ${nowPage}/${pageNumber} 
                            </c:otherwise>
                        </c:choose>
                    </c:if>
