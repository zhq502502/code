			...
			<div class="space_03">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabnew" id='tabnew'>
					<thead>
						...
						<tr>
							<td width="13%" class="tdTitle"><%=lu.getLanguage(language,"login.html.userid","用户账号") %></td>
							<td width="20%" class="tdTitle"><%=lu.getLanguage(language,"log.conf.detail.username","用户名称") %></td>
							<td width="24%" class="tdTitle"><%=lu.getLanguage(language,"log.conf.detail.logintime","进入会议室时间") %></td>
							<td width="24%" class="tdTitle"><%=lu.getLanguage(language,"log.conf.detail.logouttime","退出会议室时间") %></td>
							<td width="19%" class="tdTitle"><%=lu.getLanguage(language,"log.conf.detail.onlinetime","在线时长") %></td>
						</tr>
						</table></td></tr>
					</thead>
					<tbody>
					    ...
					    <c:forEach var="mccdf" items="${cdList}" varStatus="id">
					      <tr bgcolor="" id="${mccdf.userid }">
							<td width="13%">${mccdf.useraccount }</td>
							<td width="20%">${mccdf.username }</td>
							<td width="24%">${mccdf.userlogintime }</td>
							<td width="24%">${mccdf.userlogouttime }</td>
							<%
							if(language.equals("zh_tw")){
							%>
							<td width="19%">${fn:replace(fn:replace(fn:replace(mccdf.useronlinetime, "分钟", "分鐘"),"秒","秒"),"小时","小時")}</td>
							<%
							}else if(language.equals("en")){
							%>
							<td width="19%">${fn:replace(fn:replace(fn:replace(mccdf.useronlinetime, "分钟", "min"),"秒","s"),"小时","h")}</td>
							<%
							}else{
							%>
							<td width="19%">${mccdf.useronlinetime }</td>
							<%
							}
							%>
						  </tr>
				...
				<br />
				  <div class="sabrosus">
                    ...
                    <c:if test="${nowPage>1}">
                                <a href="MeetingCountConfDetailAction.go?begintime=${begintime}&endtime=${endtime}&confid=${confid}&page=1"><<</a>
                                <a href="MeetingCountConfDetailAction.go?begintime=${begintime}&endtime=${endtime}&confid=${confid}&page=${nowPage-1}"><</a>
                    </c:if>
                                ...
                                if(nowNumber <= 6 || pageNumber <= 10){
                                	for(int i=1;i<=10 && i<= pageNumber;i++){
                                			if(i==nowNumber){
                                                ...
                                                <a href="MeetingCountConfDetailAction.go?begintime=${begintime}&endtime=${endtime}&confid=${confid}&page=<%=i%>"><%=i %></a>
                                     		<% }
                                		}
...
%>
                    <c:if test="${nowPage!=pageNumber}">
                                <a href="MeetingCountConfDetailAction.go?begintime=${begintime}&endtime=${endtime}&confid=${confid}&page=${nowPage+1}">></a>
                                <a href="MeetingCountConfDetailAction.go?begintime=${begintime}&endtime=${endtime}&confid=${confid}&page=${pageNumber}">>></a>
                                ${nowPage}/${pageNumber} 
                    </c:if>
