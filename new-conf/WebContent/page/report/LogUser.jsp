...
<script>
    ...
    function selectUserLog(){
        ...
        document.forms[0].action = "MeetingCountUserAction.go?begintime="+btime+"&endtime="+etime;
			...
			<div class="space_03">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabnew" id="tabnew">
					<thead>
						...
						<tr>
							<td width="14%" class="tdTitle"><%=lu.getLanguage(language,"login.html.userid","用户账号") %></td>
							...
							<td width="18%" class="tdTitle"><%=lu.getLanguage(language,"log.user.attendtotaltime","参会总时长") %></td>
							<td width="16%" class="tdTitle"><%=lu.getLanguage(language,"log.user.averageonlinetime","平均参会时长") %></td>
							<td width="20%" class="tdTitle"><%=lu.getLanguage(language,"log.user.lastlogintime","最后登录时间") %></td>
					...
					class="tabcontent" id='tabcontent'>
					   ...
					   <c:forEach var="mcuf" items="${cuList}" varStatus="id">
                         <tr bgcolor="" id="${mcuf.userid}">
							<td width="14%">${mcuf.useraccount}</td>
							<td width="12%">${mcuf.username}</td>
							<td width="12%">${mcuf.useronlinenum}</td>
							<%
							if(language.equals("zh_tw")){
							%>
							<td width="18%">${fn:replace(fn:replace(fn:replace(mcuf.usertotaltime, "分钟", "分鐘"),"秒","秒"),"小时","小時")}</td>
							<td width="16%">${fn:replace(fn:replace(fn:replace(mcuf.avglogintime, "分钟", "分鐘"),"秒","秒"),"小时","小時")}</td>
							<%
							}else if(language.equals("en")){
							%>
							<td width="18%">${fn:replace(fn:replace(fn:replace(mcuf.usertotaltime, "分钟", "min"),"秒","s"),"小时","h")}</td>
							<td width="16%">${fn:replace(fn:replace(fn:replace(mcuf.avglogintime, "分钟", "min"),"秒","s"),"小时","h")}</td>
							<%
							}else{
							%>
							<td width="18%">${mcuf.usertotaltime}</td>
							<td width="16%">${mcuf.avglogintime}</td>
							<%
							}
							%>
							<td width="20%">${mcuf.logintime}</td>
							<td width="8%"><a href="CountUserDetailAction.go?begintime=${begintime}&endtime=${endtime}&uid=${mcuf.userid}"><img
               ...
               <div class="sabrosus">
                    ...
                    <c:if test="${nowPage>1}">
                                <a href="MeetingCountUserAction.go?begintime=${begintime}&endtime=${endtime}&page=1"><<</a>
                                <a href="MeetingCountUserAction.go?begintime=${begintime}&endtime=${endtime}&page=${nowPage-1}"><</a>
                    </c:if>
                                ...
                                if(nowNumber <= 6 || pageNumber <= 10){
                                	for(int i=1;i<=10 && i<= pageNumber;i++){
                                			if(i==nowNumber){
                                                ...
                                                <a href="MeetingCountUserAction.go?begintime=${begintime}&endtime=${endtime}&page=<%=i%>"><%=i %></a>
                                     		<% }
                                		}
...
%>
                    <c:if test="${nowPage!=pageNumber}">
                                <a href="MeetingCountUserAction.go?begintime=${begintime}&endtime=${endtime}&page=${nowPage+1}">></a>
                                <a href="MeetingCountUserAction.go?begintime=${begintime}&endtime=${endtime}&page=${pageNumber}">>></a>
                                ${nowPage}/${pageNumber} 
                    </c:if>
