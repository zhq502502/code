		...
		<div class="wrap">
			<div class="space_03">
               ...
               <div class="sabrosus">
                    ...
                    <c:if test="${nowPage>1}">
                                <a href="CountUserDetailAction.go?uid=${userid}&begintime=${begintime}&endtime=${endtime}&page=1"><<</a>
                                <a href="CountUserDetailAction.go?uid=${userid}&begintime=${begintime}&endtime=${endtime}&page=${nowPage-1}"><</a>
                    </c:if>
                                ...
                                if(nowNumber <= 6 || pageNumber <= 10){
                                	for(int i=1;i<=10 && i<= pageNumber;i++){
                                			if(i==nowNumber){
                                                ...
                                                <a href="CountUserDetailAction.go?uid=${userid}&begintime=${begintime}&endtime=${endtime}&page=<%=i%>"><%=i %></a>
                                     		<% }
                                		}
...
%>
                    <c:if test="${nowPage!=pageNumber}">
                                <a href="CountUserDetailAction.go?uid=${userid}&begintime=${begintime}&endtime=${endtime}&page=${nowPage+1}">></a>
                                <a href="CountUserDetailAction.go?uid=${userid}&begintime=${begintime}&endtime=${endtime}&page=${pageNumber}">>></a>
                                ${nowPage}/${pageNumber} 
                    </c:if>
