...
function selectConf(){
    ...
    document.forms[0].action = "GuestConfSelectAction.go?Keyword="+Keyword;
    document.forms[0].submit();
}
</script>
	 <form name="form" id="form" action="GuestConfListAction.go" method="post">   					
		<div class="wrap1">
			<div class="space_031">
				<div id="topLinkL">
				...
				<br />
				  <div class="sabrosus">
                    ...
                    <c:if test="${nowPage>1}">
                        <c:choose>
                            <c:when test="${Keyword!=''}">
                                <a href="GuestConfSelectAction.go?page=1&Keyword=${Keyword}"><<</a>
                                <a href="GuestConfSelectAction.go?page=${nowPage-1}&Keyword=${Keyword}"><</a>
                            </c:when>
                            <c:otherwise>
                                <a href="GuestConfListAction.go?page=1"><<</a>
                                <a href="GuestConfListAction.go?page=${nowPage-1}"><</a>
                            </c:otherwise>
                        </c:choose>
                    </c:if>
                                	...
                                	for(int i=1;i<=10 && i<= pageNumber;i++){
                                		if(Keyword != null && Keyword != "") {
                                			if(i==nowNumber){
                                    			...
                                    			out.println("<a href=GuestConfSelectAction.go?Keyword=" + Keyword + "&page=" + i + ">" + i + "</a>");
                                    		}
                                		}
                                		else {
                                			if(i==nowNumber){
                                    			...
                                    			out.println("<a href=GuestConfListAction.go?page=" + i + ">" + i + "</a>");
                                    		}
                                		}
                                	}
                                }
...
%>
                    <c:if test="${nowPage!=pageNumber}">
                        <c:choose>
                            <c:when test="${Keyword!=''}">
                                <a href="GuestConfSelectAction.go?page=${nowPage+1}&Keyword=${Keyword}">></a>
                                <a href="GuestConfSelectAction.go?page=${pageNumber}&Keyword=${Keyword}">>></a>
                                ${nowPage}/${pageNumber}
                            </c:when>
                            <c:otherwise>
                                <a href="GuestConfListAction.go?page=${nowPage+1}">></a>
                                <a href="GuestConfListAction.go?page=${pageNumber}">>></a>
                                ${nowPage}/${pageNumber} 
                            </c:otherwise>
                        </c:choose>
                    </c:if>
