		...
		<div class="wrap">
			<div class="space_03">
				...
				<div id="tabs" class="tab_m ui-tabs ui-widget ui-widget-content ui-corner-all">
					...
					<li><a href="#tabs-11"><%=lu.getLanguage(language,"confsetmanager.setusermanager","设置用户管理员") %></a></li>
					<li><a href="#tabs-12" onclick="huiyigl()"><%=lu.getLanguage(language,"confsetmanager.setconfmanager","设置会议管理员") %></a></li>
					</ul>
					...
					<div id="tabs-11">
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							...
							class="tabcontent" id='tabcontent'>
								<c:if test="${fn:length(umList)==0}"><tr><td colspan="2"><%=lu.getLanguage(language,"list.norecord","没有记录") %></td></tr></c:if>							
								...
								<tr id="${user.userId}">
									...
									<span>
										...
										<a class='removeadmin' href='javascript:void(0);'><%=lu.getLanguage(language,"usersetmanager.cancelusermanager","取消用户管理员") %></a>
									</span>
									</td>
								</tr>
		                      	</c:forEach>
						...
						<div id="sabrosus" class="sabrosus">
		                    ...
		                    <c:if test="${nowPage>1}">
		                       <a href="UserSetManagerAction.go?page=1"><<</a>
		                       <a href="UserSetManagerAction.go?page=${nowPage-1}"><</a>
		                    </c:if>
		                    <% 
			                     //总页数
		                         ...
		                         if(nowNumber <= 6 || pageNumber <= 10){
		                         	for(int i=1;i<=10 && i<= pageNumber;i++){
		                       			if(i==nowNumber){
		                           			...
		                           			out.println("<a href=UserSetManagerAction.go?page=" + i + ">" + i + "</a>");
		                           		}
		                         	}
		                         }
		                    ...
		                    <c:if test="${nowPage!=pageNumber}">
		                        <a href="UserSetManagerAction.go?page=${nowPage+1}">></a>
		                        <a href="UserSetManagerAction.go?page=${pageNumber}">>></a>
		                        ${nowPage}/${pageNumber} 
		                    </c:if>
...
<div class='message'>
		...
		<tr>
			<td width="180" >
				<div id="linkman_left" class="bd_upload" style="border: 1px solid #C6D5E1;width: 100%;height: 280px;overflow: auto;">
					<ul id="leftUl">
					      <c:forEach var="user1" items="${uumList}" varStatus="id">                                    
	...
	function search_user(){
				...
				if(confirm(getMsg(CONFIRM_CANCEL))){
					...
					$.post(siteurl+"/user/update", { action:'changeUserrole', account: account, operation: "cancel",_: new Date().getTime() } ,function(rs) {
						if(rs==0){
							...
							var page = "${nowPage}";
							location.href=siteurl+'/UserSetManagerAction.go?page='+page;
						} else {
							alert(getMsg(CANCEL_USERADMIN_FAILURE));
							return false;
						}
					});
				}
			...
			if(confirm(getMsg(CONFIRM_CANCEL))){
				$.post(siteurl+"/CheckRoleServlet", { _: new Date().getTime() } ,function(rs) {
					if(rs==null||rs==""){
						...
						$.post(siteurl+"/user/update", { action:'changeUserrole', account: usernames, operation: "cancel",_: new Date().getTime() } ,function(rs) {
							if(rs==0){
								alert(getMsg(CANCEL_USERADMIN_SUCCESS));
								var page = "${nowPage}";
								location.href=siteurl+'/UserSetManagerAction.go?page='+page;
		...
		function huiyigl(){
			var siteurl = $('#siteurl').val();
			location.href=siteurl+'/ConfPage.go?inc=ConfSetManager';
		}
	</script>
	<script type="text/javascript">
		var siteurl = $('#siteurl').val();
		...
		function selectUsersOpen(){
			$('#dialog_box').dialog({
				...
				width:455,
				height:430,
				title:"<span style='color:#FFFFFF;'>'"+getMsg(ERR_NO_USERADMIN_SELECTED)+"'</span>"
		...
		function okselect(){
			...
			var cid = $('#cid').val();
			$.post(siteurl+"/user/update", { action:'changeUserrole', account: names, operation: "add",_: new Date().getTime() } ,function(rs) {				
				location.href=siteurl+'/UserSetManagerAction.go';
            });	            
			closeDlog();
		}
		/*关闭弹出框*/
