...
<div id="wpbody-content">
	<div class="wrap">
		<div class="space_03">
			<div id="topLinkL">
				<div class="s_001"><%=lu.getLanguage(language,"confpage.menu.userinfo","个人信息") %></div>
			</div>
		<div id="uploadTip">
		...
		<form name="form" id="form" action="#" method="post" autocomplete='off'>
	      <tr>
		    ...
		    <td colspan="3"><label for="account1"><%=account %></label></td>
	      </tr>
	      <tr>
            <td style="text-align: right;"><label for="alias"><%=lu.getLanguage(language,"useradd.nickname","昵称") %></label></td>
		    <td><input name="alias" id="alias" size="22" type="text" class="Input02_w" value="<%=alias %>"/><font color="#FF0000">  *</font>
		    ...
		    <td><input name="email" id="email" size="22" type="text" class="Input02_w"  value="<%=email %>"/></td>
	      </tr>
	      <tr>
		...
		<tr>
			...
			<td style="text-align: right;"><label for="proxypass"><%=lu.getLanguage(language,"useradd.proxypass","代理登录用户密码") %></label></td>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#user_userinfo').addClass("sel_tag");
			var siteurl = $('#siteurl').val();
			$('#form').submit(function(){
				var uid = $('#userid').val();
				var account = $('#account').val();
				var alias = $('#alias').val();
				var userpass = "Itispass";//$('#userpass').val();	
				var userpass1 = "Itispass";//$('#userpass1').val();	
		        ...
		        if (proxytype != '0'&&!proxyport.match(portRegex)) {
			    	...
			    	$("#l_msg").hide();
					$.post(siteurl+"/user/update", {action:'usermod',account: account, alias: alias,uid:uid,email: email,proxytype: proxytype,proxyaddr: proxyaddr,proxyport: proxyport,proxyuser: proxyuser,proxypass: proxypass,userpass:userpass, _: new Date().getTime() } ,function(rs) {
						if(rs==0){
							alert(getMsg(MOD_SUCESS));
							location.href=siteurl+'/ConfPage.go?inc=UserInfo';
						} 
						else {
