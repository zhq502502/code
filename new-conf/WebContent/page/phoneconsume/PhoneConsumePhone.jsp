...
<div id="wpbody-content">
	 <form name="form" id="form" action="PhoneConsumeAction.go" method="post">   					
		<div class="wrap">
			<div class="space_03">
				<div id="topLinkL">
					<div class="s_001"><%=lu.getLanguage(language,"phonelog.logbyphone","通话记录-电话") %></div>
					<div class="s_002">
						...
						<input style="width:100px" type="text" id='searchKey' name="phone" maxlength='10' value="${phone}" class='Input09'>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabnew" id='tabnew'>
					<thead>
						...
						<tr>
							<td width="18%" orderName="phoneNumber"><%=lu.getLanguage(language,"phonelog.phonenumber","参会电话") %></td>
							<td width="14%" orderName="nickname"><%=lu.getLanguage(language,"phonelog.nickname","用户昵称") %></td>
							...
							<td width="13%" orderName="sumCost"><%=lu.getLanguage(language,"phonelog.sumcost","总费用") %>（<%=lu.getLanguage(language,"phonelog.yuan1","元") %>）</td>
					...
					<tr><td colspan="6"><table width="90%" border="0" cellpadding="0" cellspacing="0" class="tabcontent" id='tabcontent'>
					   ...
					   <c:forEach var="conf" items="${list}" varStatus="id">
                         <tr bgcolor="#FFFFFF" id="${conf.phoneNumber}">
                            <td width="18%">${conf.phoneNumber}</td>
                            <td width="14%">${conf.nickname}</td>
                            <%
							if(language.equals("zh_tw")){
							%>
							<td width="14%">${fn:replace(fn:replace(fn:replace(conf.sumFormatCount, "分", "分鐘"),"秒","秒"),"小时","小時")}</td>
							<%
							}else if(language.equals("en")){
							%>
							<td width="14%">${fn:replace(fn:replace(fn:replace(conf.sumFormatCount, "分", "min"),"秒","s"),"小时","h")}</td>
							<%
							}else{
							%>
                            ...
                            <td width="13%">${conf.sumCost*1.0/100}</td>
                            <td width="12%">${conf.countLogin}</td>
							<td width="10%">
							<a href="PhoneConsumeAction.go?method=phoneDetail&phone=${conf.phoneNumber}&btime=${btime}&etime=${etime}&org=${org}&loginFlag=${loginFlag}&language=<%=language %>"><%=lu.getLanguage(language,"phonelog.detail","详情") %></a>
							</td>
                        </tr>
                      </c:forEach>
				...
				<seegle:paging action="PhoneConsumeAction.go?phone=${phone}&btime=${btime}&etime=${etime}&method=phone&org=${org}&loginFlag=${loginFlag}&orderName=${orderName }&orderType=${orderType }"/>
				</c:if>
				<div align="center" >
					<input class="sbutton" value="<%=lu.getLanguage(language,"button.export","导出") %>" type="button" onclick="javascript:window.open('PhoneConsumeAction.go?phone=${phone}&btime=${btime}&etime=${etime}&method=export&type=phoneList&org=${org}&loginFlag=${loginFlag}&orderName=${orderName }&orderType=${orderType }&language=<%=language %>');"/>
					<input class="sbutton" value="<%=lu.getLanguage(language,"button.exportall","导出所有") %>" type="button" onclick="javascript:window.open('PhoneConsumeAction.go?btime=${btime}&etime=${etime}&method=export&type=phoneDetail&loginFlag=${loginFlag}&org=${org}&orderName=${orderName }&orderType=${orderType }&language=<%=language %>');"/>
				</div>
			</div>
		</div>
		<div class="clear"></div>
	...
	$('#consume_phone').addClass("sel_tag");
