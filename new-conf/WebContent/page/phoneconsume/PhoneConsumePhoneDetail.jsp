...
<div id="wpbody-content">
	 <form name="form" id="form" action="PhoneConsumeAction.go" method="post">   					
		<div class="wrap">
			<div class="space_03">
				<div id="topLinkL">
					<div class="s_001"><%=lu.getLanguage(language,"phonelog.detailbyphone","通话详情-电话") %></div>
					<div class="s_002">
						<input type="hidden" name="method" value="phoneDetail"/>
						...
						<input style="width:100px" type="hidden" id='searchKey' name="phone" maxlength='10' value="${phone}" class='Input09'>
				...
				<seegle:paging action="PhoneConsumeAction.go?phone=${phone}&btime=${btime}&etime=${etime}&method=phoneDetail&loginFlag=${loginFlag}&org=${org}&orderName=${orderName }&orderType=${orderType }"/>
				</c:if>
				<div align="center">
					<input class="sbutton" value="<%=lu.getLanguage(language,"phonelog.return","返回") %>" type="button" onclick="javascript:location.href='${url_phone}';"/>
					<input class="sbutton" value="<%=lu.getLanguage(language,"button.export","导出") %>" type="button" onclick="javascript:window.open('PhoneConsumeAction.go?phone=${phone}&btime=${btime}&etime=${etime}&method=export&type=phoneDetail&loginFlag=${loginFlag}&org=${org}&orderName=${orderName }&orderType=${orderType }&language=<%=language %>');"/>
				</div>
			</div>
		</div>
		<div class="clear"></div>
