...
public class MeetingListAction extends org.apache.struts.action.Action {
...
//    	SeegleLog.getInstance().getLog(this.getClass(), session.getAttribute("orgid").toString()).debug("debug in MeetingListAction");
        ...
        ArrayList aList = mo.queryMeetingList("", firstIndex, showNumber,token.toString(),orgid.toString());        
        if (aList.size() > 0) {
        	...
        	if(nowPage>1){
        		aList = mo.queryMeetingList("", firstIndex-showNumber, showNumber,token.toString(),orgid.toString());
            	ConfRoomActionForm crf = (ConfRoomActionForm) aList.get(0);       	
