...
//	       regkey:        注册表键名 
	...
	//检查协同是否安装
	postCheckSeegletop: function(port, regpath, regkey)
	{
		var obj = new Image();
		var url = "http://127.0.0.1:";
		url = url + port;
		...
		obj.onreadystatechange = function()
		{
			if (this.readyState == 'complete')
			{
				obj.onload  = null;
				obj.onerror = null;
				seegletop_installFlag = port;           // 客户端已经安装
				obj = null;
			}
			else
			{
				seegletop_installFlag = '1';            // 客户端未安装
			}
		}
	...
	//regpath:       注册表路径
	//regkey:        注册表键名
	startseegletop: function(regpath, regkey, exename,ip,orgid,username,password,port)
	{
		//alert("fileurl");
		var url = "http://127.0.0.1:";
		url = url + seegletop_installFlag;
		url = url + "/?type=2&regpath=";
		url = url + regpath;
		url = url + "&regkey=";
		url = url + regkey;
		...
		url = url + exename;
		url = url + "&param=";
		confpara = " -m -s " + ip + " -a " + port + " -u " + orgid+username + " -p " + password +"?/";
	    url = url + confpara;
		this.log(url);
		var objLoginExe = new Image();
		objLoginExe.src = url;
