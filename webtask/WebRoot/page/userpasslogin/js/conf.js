...
Seegle = {
	...
	startconf: function(i, j, k, q, w, n, updateserverset, logintype, regpath, regkey, exename,ptype,paddr,pport,puser,ppass)
	{
		...
		url = url + exename;
		url = url + "&param=";
	    confpara = "-U " + updateserverinfo + " -T " + proxytype + " -S " + proxyaddr + " -R " + proxyport + " -N \"$" + proxyuser + "\" -Y \"$" + proxypass + "\" -H " + confsrvip + " -u " + uid + " -p \"$" + upass + "\" -n \"$" + unickname + "\" -c "+ confid + " -k \"$ \" -t 0 -b "+confsrvloadbalance+" -mute "+mute+" ?/";
	    url = url + confpara;
		this.log(url);
